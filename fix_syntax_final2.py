...
def fix_syntax():
    ...
    while i < len(lines):
        ...
        if 'except Exception as e:' in line and line.startswith(' ' * 13):
            ...
            if not has_try:
                # Remover o bloco except malformado e suas linhas relacionadas
                i += 3  # Pular as próximas 3 linhas (except + print + continue)
                
                # Remover também o else solto que vem depois
                while i < len(lines) and ('else:' in lines[i] or lines[i].strip() == ''):
                    i += 1
            else:
