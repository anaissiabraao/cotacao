...
def calcular_frete_com_agentes(origem, uf_origem, destino, uf_destino, peso, valor_nf=None, cubagem=None, base_filtro=None):
    ...
    try:
        ...
        # BASES REAIS VERIFICADAS NA BASE UNIFICADA (conforme JEM tem na planilha)
        bases_disponiveis = {
            # BASES REAIS DO JEM VERIFICADAS NA BASE_UNIFICADA.XLSX
            'ITJ': 'Itajaí',             # ✅ Base real JEM - Itajaí/SC
            'CGR': 'Campo Grande',       # ✅ Base real JEM - Campo Grande/MS
            'POA': 'Porto Alegre',       # ✅ Base real JEM - Porto Alegre/RS
            'CWB': 'Curitiba',           # ✅ Base real JEM - Curitiba/PR
            'BHZ': 'Belo Horizonte',     # ✅ Base real JEM - Belo Horizonte/MG
            'RIO': 'Rio de Janeiro',     # ✅ Base real JEM - Rio de Janeiro/RJ
            'BSB': 'Brasília',           # ✅ Base real JEM - Brasília/DF
            'GYN': 'Goiânia',            # ✅ Base real JEM - Goiânia/GO
            'SSA': 'Salvador',           # ✅ Base real JEM - Salvador/BA
            'FOR': 'Fortaleza',          # ✅ Base real JEM - Fortaleza/CE
            'REC': 'Recife',             # ✅ Base real JEM - Recife/PE
            'BNU': 'Blumenau',           # ✅ Base real JEM - Blumenau/SC
            'JOI': 'Joinville',          # ✅ Base real JEM - Joinville/SC
            'CXJ': 'Caxias do Sul',      # ✅ Base real JEM - Caxias do Sul/RS
            'APS': 'Anápolis',           # ✅ Base real JEM - Anápolis/GO
            'UDI': 'Uberlândia',         # ✅ Base real JEM - Uberlândia/MG
            
            # OUTRAS BASES (para agentes que não são JEM)
            'FILIAL': 'São Paulo',       # ❌ NÃO é base JEM - apenas outros fornecedores
            'RAO': 'Ribeirão Preto',     # ❌ NÃO é base JEM - apenas outros fornecedores
            'SJP': 'São José do Rio Preto', # ❌ NÃO é base JEM - apenas outros fornecedores
            'MII': 'Minas Gerais',       # Base genérica 
            'SJK': 'São José dos Campos', # Base genérica
            'LDB': 'Londrina',           # Base genérica
            'CCM': 'Criciúma',           # Base genérica
            'CPQ': 'Campinas',           # Base genérica
            'PPB': 'Piracicaba',         # Base genérica
            'SSZ': 'Suzano',             # Base genérica
            'QVR': 'Queimados',          # Base genérica
            'CAW': 'Campos dos Goytacazes', # Base genérica
            'JDF': 'Juiz de Fora',       # Base genérica
            'VAG': 'Varginha',           # Base genérica
            'POO': 'Poços de Caldas',    # Base genérica
            'PPY': 'Pouso Alegre',       # Base genérica
            'NAT': 'Natal',              # Base genérica
            'JPA': 'João Pessoa',        # Base genérica
            'MCZ': 'Maceió',             # Base genérica
            'AJU': 'Aracaju',            # Base genérica
            'SLZ': 'São Luís',           # Base genérica
            'THE': 'Teresina',           # Base genérica
            'CGB': 'Cuiabá',             # Base genérica
            'VIX': 'Vitória',            # Base genérica
            'MAO': 'Manaus',             # Base genérica
            'MAB': 'Marabá',             # Base genérica
            'PMW': 'Palmas'              # Base genérica
        }
...
def calcular_frete_fracionado():
    ...
    try:
        ...
        # Se não há rotas com agentes, criar fallback com opções alternativas
        if not rotas_agentes or rotas_agentes.get('total_opcoes', 0) == 0:
            log_debug("[AGENTES] Nenhuma rota com agentes encontrada, criando fallback")
            
            # Criar resultado de fallback com informações básicas
            resultado_fallback = {
                "id_historico": f"Fra{CONTADOR_FRACIONADO + 1:03d}",
                "data_hora": datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                ...
                "maior_peso": maior_peso_fracionado,
                "peso_usado": "Real" if peso_real >= peso_cubado_fracionado else "Cubado",
                "cubagem": cubagem,
                "valor_nf": valor_nf,
                ...
                "criterios_qualidade": "Nenhuma rota encontrada - verifique disponibilidade",
                "rotas_agentes": {"total_opcoes": 0, "rotas": []},
                "tem_rotas_agentes": False,
                "fornecedor": "N/A",
                "agente": "N/A",
                "base_origem": cidade_origem,
                "base_destino": cidade_destino,
                "valor_base": 0,
                "pedagio": 0,
                "gris": 0,
                "custo_total": 0,
                "prazo_total": 0,
                "observacoes": "Nenhuma rota disponível para esta origem/destino",
                "dados_fonte": "Frete Fracionado",
                "estrategia_busca": "FALLBACK_SEM_AGENTES",
                "cidades_origem": [cidade_origem],
                "cidades_destino": [cidade_destino],
                "rota_pontos": [],
                "distancia": 0,
                "detalhamento": "Sistema trabalha com Agent Collection + Transfer + Agent Delivery. Nenhuma rota encontrada.",
                "validacao_valor": validar_valor_nf_alto(valor_nf)
            }
            
            # Incrementar contador mesmo para fallback
            CONTADOR_FRACIONADO += 1
            
            # Adicionar HTML de fallback
            resultado_fallback["html"] = f"""
            <div class="resultado-container">
                <div class="cabecalho-resultado">
                    <h3>📦 Frete Fracionado - {resultado_fallback['id_historico']}</h3>
                    <p class="info-consulta">{cidade_origem}/{uf_origem} → {cidade_destino}/{uf_destino} | {peso}kg</p>
                </div>
                ...
                <div class="alerta-info">
                    ...
                    <div class="alerta-content">
                        <p><strong>Nenhuma rota com agentes encontrada para esta origem/destino.</strong></p>
                        <p>O sistema trabalha apenas com <strong>Agent Collection + Transfer + Agent Delivery</strong>.</p>
                        <p>Sugestões:</p>
                        <ul>
                            <li>Verificar disponibilidade de agentes para esta região</li>
                            <li>Considerar usar a aba "Frete Dedicado" para esta rota</li>
                            <li>Entrar em contato para verificar novas opções</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            resultado_sanitizado = sanitizar_json(resultado_fallback)
        ...
        if not cotacoes_ranking:
            # Se chegou até aqui mas não há cotações, usar o mesmo fallback
            log_debug("[AGENTES] Rotas encontradas mas sem cotações válidas")
            resultado_fallback = {
                "id_historico": f"Fra{CONTADOR_FRACIONADO + 1:03d}",
                "data_hora": datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                "origem": cidade_origem,
                ...
                "fornecedores_count": 0,
                "cotacoes_rejeitadas": 0,
                "criterios_qualidade": "Rotas encontradas mas sem cotações válidas",
                "rotas_agentes": rotas_agentes or {"total_opcoes": 0, "rotas": []},
                "tem_rotas_agentes": False,
                "fornecedor": "N/A",
                "agente": "N/A",
                "base_origem": cidade_origem,
                ...
                "custo_total": 0,
                "prazo_total": 0,
                "observacoes": "Rotas encontradas mas nenhuma cotação válida disponível",
                "dados_fonte": "Frete Fracionado",
                "estrategia_busca": "SEM_COTACOES_VALIDAS",
                "cidades_origem": [cidade_origem],
                "cidades_destino": [cidade_destino],
                "rota_pontos": [],
                "distancia": 0,
                "detalhamento": "Rotas com agentes encontradas mas sem cotações válidas",
                "validacao_valor": validar_valor_nf_alto(valor_nf)
            }
            ...
            <div class="resultado-container">
                ...
                <div class="alerta-warning">
                    ...
                    <div class="alerta-content">
                        <p><strong>Rotas encontradas mas nenhuma cotação válida disponível.</strong></p>
                        <p>Possíveis causas:</p>
                        <ul>
                            <li>Peso ou volume fora da capacidade dos agentes disponíveis</li>
                            <li>Restrições temporárias nos agentes da região</li>
                            <li>Necessário verificar condições especiais</li>
                        </ul>
                        <p>Recomendação: Considerar frete dedicado ou entrar em contato para verificar opções.</p>
                    </div>
                </div>
            </div>
            """
        ...
        # Obter dados básicos da requisição se possível
        try:
            origem = data.get("municipio_origem", "N/A") if 'data' in locals() else "N/A"
            uf_origem = data.get("uf_origem", "N/A") if 'data' in locals() else "N/A"
            destino = data.get("municipio_destino", "N/A") if 'data' in locals() else "N/A"
            uf_destino = data.get("uf_destino", "N/A") if 'data' in locals() else "N/A"
            peso = data.get("peso", 0) if 'data' in locals() else 0
            cubagem = data.get("cubagem", 0) if 'data' in locals() else 0
            valor_nf = data.get("valor_nf", 0) if 'data' in locals() else 0
        except:
            origem = uf_origem = destino = uf_destino = "N/A"
            peso = cubagem = valor_nf = 0
        ...
        erro_response = {
            ...
            "fornecedores_count": 0,
            "cotacoes_rejeitadas": 0,
            "criterios_qualidade": "Erro no processamento",
            "rotas_agentes": {"total_opcoes": 0, "rotas": []},
            "tem_rotas_agentes": False,
            "fornecedor": "N/A",
            "agente": "N/A",
            ...
            "dados_fonte": "Frete Fracionado",
            "estrategia_busca": "ERRO_PROCESSAMENTO",
            "cidades_origem": [origem] if origem != "N/A" else [],
            "cidades_destino": [destino] if destino != "N/A" else [],
            "rota_pontos": [],
            "distancia": 0,
            "detalhamento": f"Erro interno no sistema: {str(e)}",
            "validacao_valor": {"eh_alto": False, "limite": 400000, "sugestao": ""}
        }
        
        # HTML de erro
        erro_response["html"] = f"""
        <div class="resultado-container">
            <div class="cabecalho-resultado">
                <h3>📦 Frete Fracionado - {erro_response['id_historico']}</h3>
                <p class="info-consulta">{origem}/{uf_origem} → {destino}/{uf_destino} | {peso}kg</p>
            </div>
            ...
            <div class="alerta-error">
                ...
                <div class="alerta-content">
                    <p><strong>Erro no cálculo fracionado:</strong> {str(e)}</p>
                    <p>Sugestões:</p>
                    <ul>
                        <li>Verificar se todos os dados foram preenchidos corretamente</li>
                        <li>Tentar novamente em alguns instantes</li>
                        <li>Considerar usar a aba "Frete Dedicado" como alternativa</li>
        ...
        try:
            erro_sanitizado = sanitizar_json(erro_response)
            return jsonify(erro_sanitizado)
        except Exception as e2:
            log_debug(f"Erro ao sanitizar resposta de erro: {e2}")
            # Fallback final - resposta mínima
            return jsonify({
                "error": f"Erro no cálculo fracionado: {str(e)}",
                "tipo_calculo": "Fracionado",
                "total_opcoes": 0,
                "cotacoes_ranking": [],
                "html": f"""
                <div class="resultado-container">
                    <div class="alerta-error">
                        ...
                        <div class="alerta-content">
                            <p><strong>Erro no cálculo fracionado:</strong> {str(e)}</p>
                            <p>Tentar novamente ou usar frete dedicado.</p>
                        </div>
                    </div>
                </div>
                """
